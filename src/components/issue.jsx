...
class Issue extends Component {
  ...
  render() {
    ...
    if (!issue) {
      ...
      node = (
        <IssueSimple card={card} isDragging={isDragging} onDragStart={this.onDragStart}/>
      );
    } else {
      node = (
...
// `GET .../pulls` returns an object with `mergeable` so for Pull Requests
// we have to get both.
function IssueShell(props) {
  const {card} = props;
  if (card.isLoaded()) {
    return <Issue key={card.key()} {...props}/>;
  } else {
    return (
      <Loadable
        key={card.key()}
        ...
        renderLoaded={() => <Issue key={card.key()} {...props} />}
      />
    );
  }
}
